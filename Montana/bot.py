...
@bot.command(name='makepdf', help='converts a webpage into a pdf \n' , usage="<url>")
async def convert2pdf(ctx, url, *args):
    try:
        response = requests.get(url)
        await site2pdf(ctx , url)
    except requests.ConnectionError as exception:
        raise("URL does not exist on Internet")
